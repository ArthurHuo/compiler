...
import whilelang.lang.Type.Bool;
import whilelang.lang.Type.Void;
...
public class TypeChecker {
	...
	public void check(Stmt.Assign stmt, Map<String,Type> environment) {
		...
		Type t1 = check(stmt.getLhs(),environment);
		Type t2 = check(stmt.getRhs(),environment);
		checkSubtype(t1,t2,stmt.getRhs());
	}
	...
	public void check(Stmt.Return stmt, Map<String, Type> environment) {
		...
		if(stmt.getExpr() != null) {
			Type type = check(stmt.getExpr(),environment);
			checkSubtype(function.ret,type,stmt.getExpr());// think about void.
		}else{
			Type type = new Type.Void();
			checkSubtype(function.ret,type,stmt);
		}
	}
	
	public void check(Stmt.IfElse stmt, Map<String,Type> environment) {
		Type t1 = new Type.Bool();
		Type t2 = check(stmt.getCondition(),environment);
		checkSubtype(t1,t2,stmt.getCondition());
...
//			if(file == null){
//				syntaxError("expected type " + t1 + ", found " + t2, null,
//						null);
//			}
	...
	public boolean isSubtype(Type t1, Type t2, SyntacticElement element) {	
		...
		} else if (t2 instanceof Type.Named) {	//2.3 fu za de jie jue fang an
			if((t2 instanceof Type.Named)&&(t1 instanceof Type.Named)){//shua xiao cong ming?
				if(((Type.Named) t1).getName() == ((Type.Named) t1).getName()){
					return true;
				}
			}
		...
		} else if ((t1 instanceof Type.Record) && (t2 instanceof Type.Union)){
			return true; //zhe shi wei shen me // TODO: implement me!
			
		}else if((t1 instanceof Type.Union)&&(t2 instanceof Type.Record)){
			
		
			
			Type.Union u1 = (Type.Union) t1;
			boolean isAllRecord = true;
			for(Type b1 : u1.getBounds()){
				if(!(b1 instanceof Type.Record)){
					isAllRecord = false;
				}
			}
//			
...
//				ArrayList<Type> s1 = new ArrayList<Type>();
				ArrayList<Map> t11  = new ArrayList<Map>();
				for(Type b1 : u1.getBounds()){
					Map<String,Type> l1Fields = ((Type.Record)b1).getFields();
					t11.add(l1Fields);
//					for(Map.Entry<String, Type> p : l1Fields.entrySet()){
//						Type t = (Type)p.getValue();
//						s1.add(t);
...
//				ArrayList<Type> s2 = new ArrayList<Type>();
//				Type.Union tu = null;
				Type.Record l2 = (Type.Record) t2;
				Map<String,Type> l2Fields = l2.getFields();
				
				boolean subable = true;
				
				for(int i = 0 ; i < t11.size(); i ++){
					Map<String,Type> m = t11.get(i);
					for(Map.Entry<String, Type> p : m.entrySet()){
						String s = p.getKey();
						boolean b = isSubtype(l2Fields.get(s),p.getValue(),element);
						if(!b){
							subable = false;
						}
					}
...
//				for(Map.Entry<String, Type> p : l2Fields.entrySet()){
...
//				}
//				ArrayList<Type> s3 = new ArrayList<Type>();
//				for(int i = 0; i < tu.getBounds().size(); i++){
//					s3.add(tu.getBounds().get(i));
//				}
				
//				boolean canSubtyping = true;
...
//			}
			...
			boolean flag = true;//zhe cai shi zheng que zuo fa?
