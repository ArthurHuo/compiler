...
import whilelang.lang.Expr.Binary;
...
public class Parser {
	...
	private Stmt parseStatement(boolean withSemiColon) {
		...
		if (token.text.equals("return")) {
			...
			stmt = parseWhile();
		} else if (token.text.equals("switch")){// xiu gai guo
			stmt = parseSwitch();
	...
	private Stmt parseSwitch(){ // xiu gai guo  xin jia de
		int start = index;
		matchKeyword("switch");
		match("(");
		Expr condition = parseCondition();
		match(")");
		int end = index;
		List<Stmt> blk = parseCaseBlock();
		
		
		
		return new Stmt.Switch(condition, blk, sourceAttr(start, end - 1));
	}
	...
	private Stmt parseCase(){
		...
		if(((Keyword)t).text.equals("case")){
			matchKeyword("case");
			Expr condition = parseCondition();
			match(":");
			int end = index;
			List<Stmt> blk = parseCaseStatement(true);
			
			return new Stmt.Case(condition, blk, sourceAttr(start, end - 1));
		}else if(((Keyword)t).text.equals("default")){
			matchKeyword("default");
			match(":");
			int end = index;
			List<Stmt> blk = parseCaseStatement(true);
			
			return new Stmt.Default( blk, sourceAttr(start, end - 1));
		}
		return null;
		
	}
	
	private List<Stmt> parseCaseStatement(boolean withSemiColon){
		
		ArrayList<Stmt> stmts = new ArrayList<Stmt>();
		
		while (index < tokens.size()
				&& !((tokens.get(index).text.equals("case")) || (tokens.get(index).text.equals("default")) || (tokens.get(index) instanceof RightCurly))) {
			stmts.add(parseStatement(true));
...
//			}
	...
	private Expr parseConditionExpression() {
		...
		} else if(index < tokens.size() && tokens.get(index).text.equals("is")){//xiu gai guo xin jia de 
			matchKeyword("is");
			Type rhs = parseType();
			return new Expr.Isof(Expr.BOp.INSTANCEOF,lhs,rhs,sourceAttr(start, index - 1));
		} else {
			return lhs;
		}
	}
