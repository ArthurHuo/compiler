...
public class DefiniteAssignment {
	...
	public void check(Stmt.Return stmt, Set<String> environment) {
		if(stmt.getExpr() instanceof Expr.Constant){
...
//		if(!(stmt.getExpr() instanceof Expr)){
//			flag = false;
//		}
...
//		environment.add(stmt.getName());// function jie shu hou xu yao remove diao
	}
	...
	public void check(Stmt.IfElse stmt, Set<String> environment) {
		...
		boolean trueBranch = flag;
		flag = true;
		check(stmt.getFalseBranch(), falseEnv);
		boolean falseBranch = flag;
		flag = true;
...
//		}
	...
	public void check(Expr.Invoke expr, Set<String> environment) {
		// TODO: implement me!
		List<Expr> list = expr.getArguments();
		for(int i = 0; i < list.size(); i++){
			check(list.get(i),environment);
		}
	}
	...
	public void check(Expr.RecordConstructor expr, Set<String> environment) {
		// TODO: implement me!
		List<Pair<String, Expr>> list = expr.getFields();
		for(int i = 0; i < list.size(); i++){
			check(list.get(i).second(),environment);
		}
	}
